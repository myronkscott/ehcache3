...
import org.ehcache.clustered.common.internal.exceptions.MappingException;
...
public class OffHeapServerStoreTest extends ServerStoreTest {
  ...
  public ServerStore newStore() {
    return new OffHeapServerStore(new UnlimitedPageSource(new OffHeapBufferSource()), DEFAULT_MAPPER, false, c->false);
  }
  ...
  public void testCrossSegmentShrinking() {
    ...
    try {
      OffHeapServerStore store = new OffHeapServerStore(new UpfrontAllocatingPageSource(new OffHeapBufferSource(), MEGABYTES.toBytes(1L), MEGABYTES.toBytes(1)), DEFAULT_MAPPER, false, c->false);
  ...
  public void testServerSideUsageStats() {
    ...
    OffHeapServerStore store = new OffHeapServerStore(new UpfrontAllocatingPageSource(new OffHeapBufferSource(), maxBytes, MEGABYTES.toBytes(1)), new KeySegmentMapper(16), false, c->false);
  ...
  public void testEvictionFiresEventsWithChainWhenEvictionIsEnabled() {
    OffHeapServerStore store = new OffHeapServerStore(new UpfrontAllocatingPageSource(new OffHeapBufferSource(), (long) MEGABYTES.toBytes(1), MEGABYTES.toBytes(1)), new KeySegmentMapper(16), false, c->false);
