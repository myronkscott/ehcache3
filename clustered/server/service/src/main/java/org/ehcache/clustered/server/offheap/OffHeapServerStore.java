...
public class OffHeapServerStore implements ServerStore, MapInternals {
  ...
  OffHeapServerStore(PageSource source, KeySegmentMapper mapper, boolean writeBehindConfigured, Function<Chain,Boolean> pinningCheck) {
    this.mapper = mapper;
    segments = new ArrayList<>(mapper.getSegments());
    for (int i = 0; i < mapper.getSegments(); i++) {
      if (writeBehindConfigured) {
        segments.add(new PinningOffHeapChainMap<>(source, LongPortability.INSTANCE, KILOBYTES.toBytes(4), MEGABYTES.toBytes(8), false, pinningCheck));
      } else {
  ...
  public OffHeapServerStore(ResourcePageSource source, KeySegmentMapper mapper, boolean writeBehindConfigured, Function<Chain,Boolean> pinningCheck) {
    this.mapper = mapper;
    segments = new ArrayList<>(mapper.getSegments());
    long maxSize = getMaxSize(source.getPool().getSize());
    for (int i = 0; i < mapper.getSegments(); i++) {
      if (writeBehindConfigured) {
        segments.add(new PinningOffHeapChainMap<>(source, LongPortability.INSTANCE, KILOBYTES.toBytes(4), (int) KILOBYTES.toBytes(maxSize), false, pinningCheck));
      } else {
