...
public class PinningOffHeapChainMap<K> extends OffHeapChainMap<K> {
  ...
  private final Function<Chain, Boolean> pinningCheck;

  public PinningOffHeapChainMap(PageSource source, Portability<? super K> keyPortability, int minPageSize, int maxPageSize, boolean shareByThieving, Function<Chain,Boolean> pinningCheck) {
  ...
  private boolean shouldBePinned(Chain chain) {
    return pinningCheck.apply(chain);
  }
