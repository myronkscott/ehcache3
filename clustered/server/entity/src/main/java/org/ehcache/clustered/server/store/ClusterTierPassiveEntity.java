...
public class ClusterTierPassiveEntity implements PassiveServerEntity<EhcacheEntityMessage, EhcacheEntityResponse> {
  ...
  public void createNew() throws ConfigurationException {
    stateService.createStore(storeIdentifier, configuration, false, ClusterTierServerEntityService::checkPinning);
    ...
    if (message instanceof EhcacheOperationMessage) {
      ...
      try (EhcacheStateContext ignored = stateService.beginProcessing(()->EhcacheMessageType.isTrackedOperationMessage(operationMessage.getMessageType()), storeIdentifier)) {
        ...
        if (isStoreOperationMessage(messageType)) {
          ...
          return (EhcacheEntityResponse)stateService.getStateRepositoryManager().invoke((StateRepositoryOpMessage) message, ServerStateRepositoryImpl::new);
      ...
      } catch (ClusterException | MappingException e) {
  ...
  private void invokeSyncOperation(InvokeContext context, EhcacheSyncMessage message) {
    switch (message.getMessageType()) {
      ...
      case STATE_REPO:
        ...
        stateService.getStateRepositoryManager().processSyncMessage(stateRepoSyncMessage, ServerStateRepositoryImpl::new);
        break;
