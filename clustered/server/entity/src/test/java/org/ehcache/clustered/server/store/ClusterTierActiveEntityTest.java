...
public class ClusterTierActiveEntityTest {
  ...
  public void testEnableEventListenerMessageEnablesOrDisablesEventsOnStore() throws Exception {
    ...
    when(stateService.createStore(eq(defaultStoreName), any(), anyBoolean(), any())).thenReturn(store);
  ...
  public void testLoadExistingRecoversInflightInvalidationsForEventualCache() throws Exception {
    ...
    ehcacheStateService.createStore(defaultStoreName, defaultStoreConfiguration, false, c->false);  //Passive would have done this before failover
  ...
  public void testActiveMessageTracking() throws Exception {
    ClusterTierActiveEntity activeEntity = new ClusterTierActiveEntity(defaultRegistry, defaultConfiguration, DEFAULT_MAPPER);
    EhcacheStateServiceImpl ehcacheStateService = defaultRegistry.getStoreManagerService();
    ehcacheStateService.createStore(defaultStoreName, defaultStoreConfiguration, false, c->false);  //hack to enable message tracking on active
